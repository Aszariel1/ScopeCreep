...
# The whole app is embedded via iframe on suciustefan.com (not just
# client_view), so clickjacking protection has to stay off site-wide.
...
# Cloudinary storage for user-uploaded media. Render's own disk is wiped on
# every deploy, so uploads need to live somewhere durable. Falls back to the
# local filesystem above when Cloudinary isn't configured (e.g. local dev).
